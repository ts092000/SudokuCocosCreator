        ...
        createPuzzleBoard() {
          ...
          var _loop = function _loop() {
            var _loop2 = function _loop2() {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  var selectedNode = nodeState.__selectedNode__;

                  if (selectedNode && selectedNode !== nodeSudoku) {
                    var prefabInstance = selectedNode.parent.getChildByName("framePrefab");

                    if (prefabInstance) {
                      prefabInstance.destroy();
                      delete nodeState[selectedNode.name];
                    }
                  }

                  nodeState.__selectedNode__ = nodeSudoku;

                  if (!nodeState[nodeSudoku.name]) {
                    nodeState[nodeSudoku.name] = true;

                    var _prefabInstance = instantiate(_this.frameNode);

                    _prefabInstance.name = "framePrefab";

                    _prefabInstance.setPosition(nodeSudoku.position);

                    nodeSudoku.parent.addChild(_prefabInstance);
                  }
                }
              });
            };
...
//# sourceMappingURL=551112bfd646275983f2edf3120933a1d9bf7771.js.map