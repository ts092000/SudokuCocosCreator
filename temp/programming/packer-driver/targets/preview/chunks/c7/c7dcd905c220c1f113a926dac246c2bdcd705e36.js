System.register(["__unresolved_0", "cc", "@algorithm.ts/sudoku", "__unresolved_1"], function (_export, _context) {
  ...
  var _reporterNs, _cclegacy, __checkObsolete__, __checkObsoleteInNamespace__, _decorator, Component, Node, VerticalTextAlignment, Label, Vec3, Color, Sprite, Button, director, AudioClip, AudioSource, Slider, Prefab, instantiate, SudokuCreator, GameView, _dec, _dec2, _dec3, _dec4, _dec5, _dec6, _dec7, _dec8, _dec9, _dec10, _dec11, _dec12, _dec13, _dec14, _dec15, _dec16, _dec17, _dec18, _dec19, _dec20, _dec21, _dec22, _dec23, _dec24, _dec25, _dec26, _dec27, _dec28, _dec29, _class, _class2, _descriptor, _descriptor2, _descriptor3, _descriptor4, _descriptor5, _descriptor6, _descriptor7, _descriptor8, _descriptor9, _descriptor10, _descriptor11, _descriptor12, _descriptor13, _descriptor14, _descriptor15, _descriptor16, _descriptor17, _descriptor18, _descriptor19, _descriptor20, _descriptor21, _descriptor22, _descriptor23, _descriptor24, _descriptor25, _descriptor26, _descriptor27, _descriptor28, _crd, ccclass, property, GameController;
    ...
    execute: function () {
      ...
      _export("GameController", GameController = (_dec = ccclass('GameController'), _dec2 = property(_crd && GameView === void 0 ? (_reportPossibleCrUseOfGameView({
        ...
        start() {
          ...
          var saveVolumeBg = localStorage.getItem('volumeBg');
          ...
          var saveVolumeEffect = localStorage.getItem('volumeEffect');
        ...
        volumeGameSetting() {
          ...
          var progressBg = this.settingVolumeBg.progress;
        ...
        volumeGameEffect() {
          ...
          var progressEffect = this.settingVolumeEffect.progress;
        ...
        getRowAndColumn(node) {
          var x = node.position.x - this.boardNode.position.x;
          var y = node.position.y - this.boardNode.position.y;
          var row = Math.floor(y / 55);
          var column = Math.floor(x / 55);
          return {
            row,
            column
          };
        ...
        getNodesInGrid(node) {
          ...
          var gridColumn = Math.floor(column / 3) * 3;

          for (var i = gridRow; i < gridRow + 3; i++) {
            for (var j = gridColumn; j < gridColumn + 3; j++) {
              if (i !== row || j !== column) {
        ...
        createPuzzleBoard() {
          ...
          var _loop = function _loop(i) {
            ...
            var _loop2 = function _loop2(j) {
              ...
              //         }else {
              //             nodeState[nodeSudoku.name] = false;
              //         }
              //     }
              // });
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  ...
                  if (!nodeState[nodeSudoku.name]) {
                    ...
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance); // Show the prefab on the same row

                    var row = Math.floor(nodeSudoku.position.x / 55);

                    for (var _i3 = 0; _i3 < 9; _i3++) {
                      if (_i3 !== row) {
                        var node = _this.boardNode.getChildByName("ValueGame" + (_i3 * 9 + j));

                        var nodePrefab = instantiate(_this.frameNode);
                    ...
                    for (var _j = 0; _j < 9; _j++) {
                      if (_j !== col) {
                        var _node = _this.boardNode.getChildByName("ValueGame" + (i * 9 + _j));

                        var _nodePrefab = instantiate(_this.frameNode);
                    ...
                    for (var _i4 = rowStart; _i4 < rowStart + 3; _i4++) {
                      for (var _j2 = colStart; _j2 < colStart + 3; _j2++) {
                        if (_i4 !== row && _j2 !== col) {
                          var _node2 = _this.boardNode.getChildByName("ValueGame" + (_i4 * 9 + _j2));

                          var _nodePrefab2 = instantiate(_this.frameNode);
                    ...
                    // Remove the prefab from all nodes
                    nodeState[nodeSudoku.name] = false;

                    for (var _i5 = 0; _i5 < 9; _i5++) {
                      for (var _j3 = 0; _j3 < 9; _j3++) {
                        var _node3 = _this.boardNode.getChildByName("ValueGame" + (_i5 * 9 + _j3));

                        var prefabNode = _node3.getChildByName("PrefabNode");
              ...
              nodeNum.on(Node.EventType.TOUCH_END, () => {
                if (_this.selectedNode) {
                  ...
                  if (!isNaN(selectedValue)) {
                    ...
                    var _i6 = Math.floor(_this.selectedNode.position.x / 55);

                    var _j4 = Math.floor(_this.selectedNode.position.y / 55);

                    var position = _i6 * 9 + _j4;
                    var solutionValue = _this.solutionGame[position];
                    var isCorrect = selectedValue === solutionValue;
                    console.log(isCorrect);

                    var labelBlockSelect = _this.selectedNode.getComponent(Label);

                    if (!nodeCheck[_i6][_j4].filled) {
                      if (isCorrect) {
                        labelBlockSelect.color = Color.BLUE;
                        emtyNode--;
                        nodeCheck[_i6][_j4].filled = true;
                        console.log(emtyNode);
                        ...
                        if (emtyNode === 0) {
                          ...
                          _this.bgMain.getComponent(AudioSource).pause();
...
//# sourceMappingURL=c7dcd905c220c1f113a926dac246c2bdcd705e36.js.map