            ...
            var _loop = function _loop() {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  ...
                  if (!prefabInstances[nodeSudoku.name]) {
                    prefabInstances[nodeSudoku.name] = [];
                  }

                  if (prefabInstances[nodeSudoku.name].length === 0) {
                    var prefabInstance = instantiate(_this.frameNode);
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                    prefabInstances[nodeSudoku.name].push(prefabInstance);
                  } else {
                    // Xóa bỏ prefab cũ trên node cũ
                    prefabInstances[_this.selectedNode.name].forEach(instance => {
                      instance.destroy();
                    }); // Tạo mới prefab trên node mới
                    ...
                    prefabInstances[nodeSudoku.name].push(_prefabInstance);
                  }
                }
              });
...
//# sourceMappingURL=a68a1fe2fcb55f4cce28a1473c5cadb726c82216.js.map