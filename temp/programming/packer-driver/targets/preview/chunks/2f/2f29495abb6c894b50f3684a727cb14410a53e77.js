        ...
        createPuzzleBoard() {
          ...
          for (var _i2 = 0; _i2 < 9; _i2++) {
            ...
            var _loop = function _loop() {
              ...
              var nodeSudoku = new Node("ValueGame" + (_i2 * 9 + j));
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  var nodeName = nodeSudoku.name;
                  var prevPrefab = nodePrefabs[nodeName];

                  if (prevPrefab) {
                    // If a prefab is already attached to the node, detach it
                    prevPrefab.removeFromParent();
                    nodePrefabs[nodeName] = null;
                  } else {
                    // If there is no prefab attached, create and attach one
                    var prefabInstance = instantiate(_this.frameNode);
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                    nodePrefabs[nodeName] = prefabInstance;
                  }
                }
              });
...
//# sourceMappingURL=2f29495abb6c894b50f3684a727cb14410a53e77.js.map