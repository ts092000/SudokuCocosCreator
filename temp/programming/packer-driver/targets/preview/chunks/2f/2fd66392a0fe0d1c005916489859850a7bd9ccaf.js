          ...
          var _loop = function _loop() {
            ...
            nodeNum.on(Node.EventType.TOUCH_END, () => {
              if (_this.selectedNode) {
                ...
                var selectedValue = parseInt(label.string); // Convert the label's string value to a number

                if (!isNaN(selectedValue)) {
                  // Check if the selected value is a valid number
                  var _i3 = Math.floor(_this.selectedNode.position.x / 55); // Calculate the i index based on the selected node's position


                  var _j = Math.floor(_this.selectedNode.position.y / 55); // Calculate the j index based on the selected node's position


                  var position = _i3 * 9 + _j; // Calculate the position in the array based on the i and j indices
...
//# sourceMappingURL=2fd66392a0fe0d1c005916489859850a7bd9ccaf.js.map