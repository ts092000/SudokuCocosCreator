        ...
        createPuzzleBoard() {
          ...
          for (var _i2 = 0; _i2 < 9; _i2++) {
            ...
            var _loop = function _loop() {
              ...
              var nodeState = {};
              var prefabInstance = null;
              var offset = new Vec3(0, 0, 0);
              nodeSudoku.on(Node.EventType.TOUCH_START, event => {
                if (value === 0) {
                  _this.selectedNode = nodeSudoku;

                  if (!nodeState[nodeSudoku.name]) {
                    nodeState[nodeSudoku.name] = true;
                    prefabInstance = instantiate(_this.frameNode);
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                  }

                  offset = nodeSudoku.convertToNodeSpaceAR(event.getLocation());
                }
              });
              nodeSudoku.on(Node.EventType.TOUCH_MOVE, event => {
                if (value === 0 && prefabInstance) {
                  var touchPos = event.getLocation();
                  var newPos = nodeSudoku.convertToNodeSpaceAR(touchPos);
                  prefabInstance.setPosition(newPos.sub(offset));
                }
              });
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                prefabInstance = null;
...
//# sourceMappingURL=ff3a5417c550f5e5b143ad6ed88065daf1d64a42.js.map