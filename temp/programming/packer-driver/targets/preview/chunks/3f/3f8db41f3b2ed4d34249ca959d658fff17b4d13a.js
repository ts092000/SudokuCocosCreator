        ...
        createPuzzleBoard() {
          ...
          for (var _i2 = 0; _i2 < 9; _i2++) {
            ...
            var _loop = function _loop() {
              ...
              var nodeState = {
                selectedNode: null,
                // Lưu trạng thái node đã được chọn trước đó
                prefabInstance: null // Lưu trạng thái prefab được tạo

              };
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  if (nodeState.selectedNode && nodeState.prefabInstance) {
                    // Kiểm tra xem node trước đó có prefab không
                    nodeState.prefabInstance.removeFromParent(); // Xóa prefab

                    nodeState.prefabInstance = null; // Reset prefabInstance
                  }
                  ...
                  if (!nodeState[nodeSudoku.name]) {
                    nodeState[nodeSudoku.name] = true;
                    nodeState.prefabInstance = instantiate(_this.frameNode);
                    nodeState.prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(nodeState.prefabInstance);
                  }

                  nodeState.selectedNode = nodeSudoku; // Lưu trạng thái node đã được chọn
                }
              });
              ...
              nodeNum.position = new Vec3(_i2 % 3 * 100 + 80, Math.floor(_i2 / 3) * -60, 0);
...
//# sourceMappingURL=3f8db41f3b2ed4d34249ca959d658fff17b4d13a.js.map