        ...
        createPuzzleBoard() {
          ...
          for (var _i2 = 0; _i2 < 9; _i2++) {
            ...
            var _loop = function _loop() {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  // Check if there is a selectedPrefab
                  if (selectedPrefab) {
                    // Move the selectedPrefab to the new node
                    selectedPrefab.setPosition(nodeSudoku.position);
                    selectedPrefab = null; // Deselect the prefab
                  } else {
                    // Create a new prefab at the new node
                    if (!nodeState[nodeSudoku.name]) {
                      nodeState[nodeSudoku.name] = true;
                      var prefabInstance = instantiate(_this.frameNode);
                      prefabInstance.setPosition(nodeSudoku.position);
                      nodeSudoku.parent.addChild(prefabInstance);
                      selectedPrefab = prefabInstance; // Select the new prefab
                    }
                  }
                }
              });
...
//# sourceMappingURL=37afcf63d348c2f45dc1777c8d10f7627df97829.js.map