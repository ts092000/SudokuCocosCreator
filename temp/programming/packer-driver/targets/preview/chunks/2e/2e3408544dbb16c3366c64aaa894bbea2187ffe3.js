              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  ...
                  if (!nodeState[nodeName]) {
                    nodeState[nodeName] = true; //Instantiate the prefab

                    var prefabInstance = instantiate(_this.frameNodeNum);
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                    var row = Math.floor(nodeSudoku.position.x / 55); // Create prefabs for the row
                    ...
                    for (var _i3 = 0; _i3 < 9; _i3++) {
                      if (_i3 !== row) {
                        ...
                        nodePrefab.setPosition(node.position);
                        node.parent.addChild(nodePrefab); // Add the prefab to the rowPrefabs object

                        if (!rowPrefabs[row]) {
                          rowPrefabs[row] = [];
                        }

                        rowPrefabs[row].push(nodePrefab);
                      }
                    } // Create prefabs for the column
                    ...
                    } // Create prefabs for the 3x3 block
                  ...
                  // Remove the prefabs for the row if they exist
                  var _row = Math.floor(nodeSudoku.position.x / 55);

                  if (rowPrefabs[_row]) {
                    for (var _i5 = 0; _i5 < rowPrefabs[_row].length; _i5++) {
                      var prefab = rowPrefabs[_row][_i5];
                      prefab.destroy();
                    }

                    delete rowPrefabs[_row];
                  }
                }
              });
              ...
              nodeNum.on(Node.EventType.TOUCH_END, () => {
                if (_this.selectedNode) {
                  ...
                  if (!isNaN(selectedValue)) {
                    ...
                    if (!nodeCheck[_i6][_j3].filled) {
                      if (isCorrect) {
                        labelBlockSelect.color = Color.BLUE;
                        emtyNode--;
                        nodeCheck[_i6][_j3].filled = true;
                        console.log(emtyNode);
...
//# sourceMappingURL=2e3408544dbb16c3366c64aaa894bbea2187ffe3.js.map