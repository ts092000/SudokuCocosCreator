        ...
        createPuzzleBoard() {
          ...
          for (var _i2 = 0; _i2 < 9; _i2++) {
            ...
            var _loop = function _loop() {
              ...
              var selectedNode = null; // Khởi tạo biến selectedNode để lưu trữ node đang được chọn

              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  if (selectedNode !== null && selectedNode !== nodeSudoku) {
                    // Kiểm tra xem node cũ đã được chọn có khác với node mới không
                    var oldPrefabInstance = selectedNode.getChildByName("PrefabInstance"); // Lấy prefabInstance của node cũ

                    if (oldPrefabInstance !== null) {
                      // Kiểm tra xem node cũ đã được hiển thị prefabInstance chưa
                      oldPrefabInstance.removeFromParent(); // Nếu có, xóa prefabInstance khỏi node cũ
                    }
                  }

                  selectedNode = nodeSudoku; // Cập nhật node được chọn

                  var prefabInstance = instantiate(_this.frameNode);
                  prefabInstance.setPosition(nodeSudoku.position);
                  prefabInstance.name = "PrefabInstance"; // Đặt tên cho prefabInstance để dễ dàng tìm kiếm và xóa sau này

                  nodeSudoku.addChild(prefabInstance); // Hiển thị prefabInstance cho node mới được chọn
                }
              });
              ...
              nodeNum.position = new Vec3(_i2 % 3 * 70 + 210, Math.floor(_i2 / 3) * -60, 0);
...
//# sourceMappingURL=d4eca75e5b841ab62b2b1f36d01d37982b2925df.js.map