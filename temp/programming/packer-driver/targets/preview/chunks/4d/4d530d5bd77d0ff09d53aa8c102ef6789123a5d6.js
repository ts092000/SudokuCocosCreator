            ...
            var _loop = function _loop() {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  ...
                  if (!nodeState[nodeSudoku.name]) {
                    // Nếu node chưa được chạm trước đó
                    nodeState[nodeSudoku.name] = true;
                    var prefabInstance = instantiate(_this.frameNode);
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                  } else {
                    // Nếu node đã được chạm trước đó
                    var _prefabInstance = nodeSudoku.parent.getChildByName(nodeSudoku.name);

                    if (_prefabInstance) {
                      // Nếu prefab cũ tồn tại thì di chuyển nó đến node mới
                      _prefabInstance.setPosition(nodeSudoku.position);

                      nodeSudoku.parent.addChild(_prefabInstance);
                    } else {
                      // Nếu prefab cũ không tồn tại thì tạo prefab mới và thêm vào node mới
                      nodeState[nodeSudoku.name] = true;
                      var newPrefabInstance = instantiate(_this.frameNode);
                      newPrefabInstance.setPosition(nodeSudoku.position);
                      nodeSudoku.parent.addChild(newPrefabInstance);
                    }
                  }
                }
              });
...
//# sourceMappingURL=4d530d5bd77d0ff09d53aa8c102ef6789123a5d6.js.map