        ...
        createPuzzleBoard() {
          ...
          for (let i = 0; i < 9; i++) {
            for (let j = 0; j < 9; j++) {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  const selectedNode = nodeState.__selectedNode__;

                  if (selectedNode && selectedNode !== nodeSudoku) {
                    const prefabInstance = selectedNode.parent.getChildByName("framePrefab");
                  ...
                  if (!nodeState[nodeSudoku.name]) {
                    nodeState[nodeSudoku.name] = true;
                    const prefabInstance = instantiate(this.frameNode);
                    prefabInstance.name = "framePrefab";
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                  }
                }
...
//# sourceMappingURL=5bfc29f73115d6cf8dee311b8d4c467de735a478.js.map