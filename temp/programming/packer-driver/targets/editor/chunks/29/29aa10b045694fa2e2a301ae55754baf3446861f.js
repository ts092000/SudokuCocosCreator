              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  ...
                  if (!nodeState[nodeName]) {
                    ...
                    prefabInstance.setPosition(nodeSudoku.position);
                    nodeSudoku.parent.addChild(prefabInstance);
                    const row = Math.floor(nodeSudoku.position.x / 55); // Create prefabs for the row
                  ...
                  if (rowPrefabs[row]) {
                    for (let i = 0; i < rowPrefabs[row].length; i++) {
                      const prefab = rowPrefabs[row][i];
                      prefab.destroy();
                    }

                    delete rowPrefabs[row];
                  }
                }
              });
...
//# sourceMappingURL=29aa10b045694fa2e2a301ae55754baf3446861f.js.map