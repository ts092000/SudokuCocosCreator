        ...
        createPuzzleBoard() {
          ...
          for (let i = 0; i < 9; i++) {
              ...
              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  if (this.selectedNode) {
                    const prevLabelBlock = previousNode.getComponent(Label);
                    prevLabelBlock.string = this.puzzleGame[previousNode.gridIndex] === 0 ? "    " : this.puzzleGame[previousNode.gridIndex].toString();
                  }
            ...
            nodeNum.on(Node.EventType.TOUCH_END, () => {
              if (this.selectedNode) {
                const selectedValue = parseInt(label.string); // if (!isNaN(selectedValue)) {
...
//# sourceMappingURL=0b5068f24f98ee7dca83de2b62193f0349d9298e.js.map