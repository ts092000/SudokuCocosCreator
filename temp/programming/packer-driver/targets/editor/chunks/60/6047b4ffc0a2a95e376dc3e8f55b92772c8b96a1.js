        ...
        createPuzzleBoard() {
          ...
          for (let i = 0; i < 9; i++) {
            for (let j = 0; j < 9; j++) {
              ...
              let emptyString = "    "; // Khoảng trắng có độ dài bằng 4 ký tự

              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  if (this.selectedNode) {
                    const prevLabelBlock = this.selectedNode.getComponent(Label);
                    const prevValue = prevLabelBlock.string.trim();
                  ...
                  if (this.selectedNode && this.selectedNode !== nodeSudoku) {
                    const prevLabelBlock = this.selectedNode.getComponent(Label);
                    const prevValue = prevLabelBlock.string.trim();
                    prevLabelBlock.string = prevValue === emptyString ? "aaa" : prevValue;
                    this.selectedNode = null;
                ...
                if (!isNaN(selectedValue)) {
                  ...
                  if (!isCorrect) {
                    ...
                    if (this.redNumber === 4) {
                      ...
                      console.log(bestTime);
                      this.bestTime.string = 'best Time: ' + bestTime + 'giây';
                    }
                  } else {
                    labelBlock.color = Color.BLUE;
                    emtyNode -= 1;
...
//# sourceMappingURL=6047b4ffc0a2a95e376dc3e8f55b92772c8b96a1.js.map