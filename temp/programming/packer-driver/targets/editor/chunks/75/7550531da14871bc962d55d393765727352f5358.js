        ...
        createPuzzleBoard() {
          ...
          for (let i = 0; i < 9; i++) {
            ...
            for (let j = 0; j < 9; j++) {
              ...
              let selectedNode = null; // Khởi tạo biến selectedNode để lưu trữ node đang được chọn

              nodeSudoku.on(Node.EventType.TOUCH_END, () => {
                if (value === 0) {
                  if (selectedNode !== null && selectedNode !== nodeSudoku) {
                    // Kiểm tra xem node cũ đã được chọn có khác với node mới không
                    const oldPrefabInstance = selectedNode.getChildByName("PrefabInstance"); // Lấy prefabInstance của node cũ
...
//# sourceMappingURL=7550531da14871bc962d55d393765727352f5358.js.map